...
(() => {
  ...
  function sellrocketExtractOrder() {
    ...
    const products = extractOrderProductsFromDom();
  ...
  function parseSku(raw) {
    ...
    const m = txt.match(/SKU\s*:\s*([A-Z0-9._\-\/]+)/i);
    if (m) return m[1].trim();
    return txt;
  }

  function parseEanFromRowText(txt) {
    const t = normalizeSpaces(txt || "");
    const m = t.match(/EAN\s*:\s*([0-9]+)/i);
    return m ? m[1].trim() : "";
  }

  function parseQty(raw) {
    const t = normalizeSpaces(raw || "").replace(",", ".").trim();
    const m = t.match(/(\d+(?:\.\d+)?)/);
    if (!m) return 0;
  ...
  function extractOrderProductsFromDom() {
    // Produkty są w MuiDataGrid (wirtualizacja) — zczytujemy to, co jest aktualnie w DOM.
    // Jeśli wiersze nie są jeszcze załadowane, użytkownik musi przewinąć tabelę.
    const rows = Array.from(document.querySelectorAll(".MuiDataGrid-row[role='row'][data-id]"));
    const out = [];
    const seen = new Set();

    for (const row of rows) {
      const platformId = String(row.getAttribute("data-id") || "").trim();
      const nameEl = row.querySelector(
        "[data-testid='order_detail_productName'] span, [data-testid='order_detail_productName'] p span, [data-testid='order_detail_productName']"
      );
      const qtyEl = row.querySelector("[data-testid='order_detail_productQuantity']");
      const skuEl = row.querySelector("[data-testid='order_detail_productSKU']");

      const rawName = normalizeSpaces(nameEl?.textContent || "");
      const skuRaw = skuEl?.textContent || "";
      const sku = parseSku(skuRaw);
      const qty = parseQty(qtyEl?.textContent || "");
      const ean = parseEanFromRowText(row.textContent || "");

      const name = rawName;
      const key = platformId || sku || name;
      if (!key) continue;
      if (seen.has(key)) continue;
      if (!name && !sku) continue;

      out.push({ platformId, sku, name, ean, qty });
      seen.add(key);
    }

    // Limit dla bezpieczeństwa długości URL (i przypadków z wieloma pozycjami).
  ...
  function buildReturnUrlWithData(data) {
    try {
      ...
      if (Array.isArray(data.products) && data.products.length) {
        ...
        url.searchParams.set("mr_products", JSON.stringify(min));
      }
      return url.toString();
    } catch {
      return "";
  ...
  // Linki do zamówienia: /unified-orders/12345 (preferowane z wierszy tabeli)
  function getOrderLinks() {
    const isOrderHref = (href) => /\/unified-orders\/\d+/.test(href || "");
    const all = Array.from(document.querySelectorAll("a[href*='/unified-orders/']")).filter((a) =>
      isOrderHref(a.getAttribute("href"))
    );
    const fromTable = all.filter((a) => a.closest("table tbody tr, [role='row'], .ag-row"));
    return fromTable.length > 0 ? fromTable : all;
  }

  // Czeka na wyniki, potem klika w pierwsze zamówienie (tylko z Niezbędnika).
  function waitAndOpenFirstResult() {
    if (isSellrocketOrderView()) return;
    const POLL_MS = 500;
    ...
    function tryClick() {
      ...
      elapsed += POLL_MS;
      if (elapsed < MAX_WAIT_MS) setTimeout(tryClick, POLL_MS);
      else toast("Nie znaleziono wyników w czasie. Wejdź w zamówienie ręcznie i kliknij „Zgłoszenie: kopiuj dane”.");
    }
  ...
  async function main() {
    ...
    const hadQuery = await sellrocketMaybeAutoSearch(); // true gdy był mr_q w URL
    sellrocketInjectCopyButton();

    if (isSellrocketOrderView()) {
      setTimeout(autoCopyOnOrderView, 600);
    } else {
      // Auto‑otwarcie pierwszego zamówienia: gdy był mr_q LUB po przekierowaniu sesja jest w GM
      const fromReklamacje = hadQuery || !!GM_getValue(KEYS.autoCopySession, null);
      if (fromReklamacje) waitAndOpenFirstResult();
    }
  }
